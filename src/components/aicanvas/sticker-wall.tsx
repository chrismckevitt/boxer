...
function measureTextCard(
    ...
    ctx.save()
    ctx.font = `600 ${TEXT_FONT_PX}px 'Space Grotesk Variable', ui-sans-serif, system-ui, sans-serif`
...
export default function StickerWall() {
    ...
    useEffect(() => {
        ...
        function drawFrame(now: number) {
            ...
            for (const s of stickers) {
                ...
                ctx!.strokeStyle = isDarkRef.current ? 'rgba(255,255,255,0.15)' : 'rgba(17,15,12,0.1)'
    ...
    const accentBg = isDark ? '#FFFFFF' : '#110F0C'
    const inputText = isDark ? 'rgba(255,255,255,0.95)' : 'rgba(17,24,39,0.95)'
    const accentText = isDark ? '#110F0C' : '#FFFFFF'
    ...
    return (
        <div
            ...
            style={{background: bg, height: '600px'}}
        >
            <motion.div
                ref={containerRef}
                initial={{opacity: 0}}
                ...
                style={{background: bg, touchAction: 'none', height: '600px'}}
            >
                <canvas
                    ref={canvasRef}
