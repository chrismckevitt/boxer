...
// The component is always in dark mode at rest — dark bg, light text.
// On hover it inverts to light mode in sync with the word reveal.
const DARK_BG = '#110F0C'
const LIGHT_BG = '#FFFFFF'
const DARK_FG = '#FFFFFF'
const LIGHT_FG = '#110F0C'
...
const WORD_TOP = 'ON-GRID'
const WORD_BOTTOM = 'OFF-GRID'
...
export default function SliceType() {
  ...
  // Top prefix lives in its own absolutely-positioned wrapper pinned to the
  // container's LEFT edge. The shared tail lives pinned to the RIGHT edge. At
  // rest the prefix overlaps the bottom word's prefix. When the hybrid opens,
  // the prefix translates right to its natural position so the top word reads
  // cleanly.
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      // Measure the ink-level left edge for each prefix at the current font so
      // the nudge is accurate on every screen size and device.
      const computed = window.getComputedStyle(lEl)
      const font = `900 ${computed.fontSize} ${computed.fontFamily}`
      const topInk = measureLeftInk(TOP_PREFIX.charAt(0), font)
      const botInk = measureLeftInk(WORD_BOTTOM.charAt(0), font)
      nudgeMV.set(botInk - topInk)
    }
    measure()
  ...
  // The shared tail — characters identical between both words, right-anchored.
  const SHARED_SUFFIX_LEN = 5 // "-GRID"
  const TAIL = WORD_TOP.slice(-SHARED_SUFFIX_LEN)
  const TOP_PREFIX = WORD_TOP.slice(0, -SHARED_SUFFIX_LEN)

  return (
    <motion.div
      ref={rootRef}
      className="flex h-full w-full items-center justify-center overflow-hidden"
      style={{ backgroundColor: bgColor, color: fgColor, touchAction: 'manipulation', cursor: 'default' }}
    ...
    >
      ...
      >
        ...
        >
          {/* Top prefix — pinned left at rest, translates right on engage. */}
          <motion.span
            ref={lRef}
            style={{
              ...sharedTextStyle,
